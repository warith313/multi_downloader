...
void main() async {
  ...
  await MultiDownloader.initialize();
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initState() {
    ...
    MultiDownloader.registerCallback(downloadCallback);
  ...
  void _requestDownload(_TaskInfo task) async {
    task.taskId = await MultiDownloader.enqueue(
        url: task.link,
  ...
  void _cancelDownload(_TaskInfo task) async {
    await MultiDownloader.cancel(taskId: task.taskId);
  }

  void _pauseDownload(_TaskInfo task) async {
    await MultiDownloader.pause(taskId: task.taskId);
  }

  void _resumeDownload(_TaskInfo task) async {
    String newTaskId = await MultiDownloader.resume(taskId: task.taskId);
    task.taskId = newTaskId;
  }

  void _retryDownload(_TaskInfo task) async {
    String newTaskId = await MultiDownloader.retry(taskId: task.taskId);
    task.taskId = newTaskId;
  }

  Future<bool> _openDownloadedFile(_TaskInfo task) {
    return MultiDownloader.open(taskId: task.taskId);
  }

  void _delete(_TaskInfo task) async {
    await MultiDownloader.remove(
  ...
  Future<Null> _prepare() async {
    final tasks = await MultiDownloader.loadTasks();
