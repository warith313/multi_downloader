...
public class DownloadWorker extends Worker implements MethodChannel.MethodCallHandler {
    ...
    public static final String ARG_SRT = "srt";
    ...
    private void startBackgroundIsolate(Context context) {
        synchronized (isolateStarted) {
            if (backgroundFlutterView == null) {
                SharedPreferences pref = context.getSharedPreferences(MultiDownloaderPlugin.SHARED_PREFERENCES_KEY, Context.MODE_PRIVATE);
                long callbackHandle = pref.getLong(MultiDownloaderPlugin.CALLBACK_DISPATCHER_HANDLE_KEY, 0);
        ...
        backgroundChannel = new MethodChannel(backgroundFlutterView, "com.iraqtechno/downloader_background");
    ...
    public Result doWork() {
        ...
        String image = getInputData().getString(ARG_IMAGE);
        String srt = getInputData().getString(ARG_SRT);
        String name = getInputData().getString(ARG_NAME);
        ...
        msgStarted = res.getString(R.string.multi_downloader_notification_started);
        msgInProgress = res.getString(R.string.multi_downloader_notification_in_progress);
        msgCanceled = res.getString(R.string.multi_downloader_notification_canceled);
        msgFailed = res.getString(R.string.multi_downloader_notification_failed);
        msgPaused = res.getString(R.string.multi_downloader_notification_paused);
        msgComplete = res.getString(R.string.multi_downloader_notification_complete);

        Log.d(TAG, "DownloadWorker{url=" + url + ",filename=" + filename + ",image=" + image + ",srt=" + srt + ",name=" + name + ",savedDir=" + savedDir + ",header=" + headers + ",isResume=" + isResume);
