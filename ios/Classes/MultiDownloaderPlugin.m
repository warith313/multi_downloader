#import "MultiDownloaderPlugin.h"
#import "DBManager.h"
...
@interface MultiDownloaderPlugin()<NSURLSessionTaskDelegate, NSURLSessionDownloadDelegate, UIDocumentInteractionControllerDelegate>
{
...
@implementation MultiDownloaderPlugin

static MultiDownloaderPlugin *instance = nil;
...
{
    if (self = [super init]) {
        _headlessRunner = [[FlutterEngine alloc] initWithName:@"MultiDownloaderIsolate" project:nil allowHeadlessExecution:YES];
        _registrar = registrar;

        _mainChannel = [FlutterMethodChannel
                           methodChannelWithName:@"com.iraqtechno/downloader"
                           binaryMessenger:[registrar messenger]];
        [registrar addMethodCallDelegate:self channel:_mainChannel];

        _callbackChannel =
        [FlutterMethodChannel methodChannelWithName:@"com.iraqtechno/downloader_background"
                                    binaryMessenger:_headlessRunner];

        _eventQueue = [[NSMutableArray alloc] init];

        NSBundle *frameworkBundle = [NSBundle bundleForClass:MultiDownloaderPlugin.class];

        // initialize Database
        NSURL *bundleUrl = [[frameworkBundle resourceURL] URLByAppendingPathComponent:@"MultiDownloaderDatabase.bundle"];
        NSBundle *resourceBundle = [NSBundle bundleWithURL:bundleUrl];
        NSString *dbPath = [resourceBundle pathForResource:@"download_tasks" ofType:@"sql"];
        NSLog(@"database path: %@", dbPath);
        databaseQueue = dispatch_queue_create("com.iraqtechno.multi_downloader", 0);
...
- (NSURLSessionDownloadTask*)downloadTaskWithURL: (NSURL*) url fileName: (NSString*) fileName andImage: (NSString*) image andSrt: (NSString*) srt andName: (NSString*) name andSavedDir: (NSString*) savedDir andHeaders: (NSString*) headers
{
...
- (NSURL*)fileUrlFromDict:(NSDictionary*)dict
{
    ...
    NSString *image = dict[KEY_IMAGE];
    NSString *srt = dict[KEY_SRT];
...
- (NSURL*)imageUrlFromDict:(NSDictionary*)dict
{
    NSString *url = dict[KEY_URL];
    NSString *savedDir = dict[KEY_SAVED_DIR];
    NSString *filename = dict[KEY_FILE_NAME];
    ...
    NSLog(@"filename: %@", filename);
    NSURL *savedDirURL = [NSURL fileURLWithPath:savedDir];
    return [savedDirURL URLByAppendingPathComponent:image];
}

- (NSURL*)srtUrlFromDict:(NSDictionary*)dict
{
    NSString *url = dict[KEY_URL];
    NSString *savedDir = dict[KEY_SAVED_DIR];
    NSString *filename = dict[KEY_FILE_NAME];
    ...
    NSLog(@"filename: %@", filename);
    NSURL *savedDirURL = [NSURL fileURLWithPath:savedDir];
    return [savedDirURL URLByAppendingPathComponent:srt];
}
...
- (void) addNewTask: (NSString*) taskId url: (NSString*) url status: (int) status progress: (int) progress filename: (NSString*) filename image: (NSString*) image srt: (NSString*) srt name: (NSString*) name savedDir: (NSString*) savedDir headers: (NSString*) headers resumable: (BOOL) resumable showNotification: (BOOL) showNotification openFileFromNotification: (BOOL) openFileFromNotification
{
...
- (NSDictionary*) taskDictFromRecordArray:(NSArray*)record
{
    ...
    NSString *image = [record objectAtIndex:[_dbManager.arrColumnNames indexOfObject:@"image"]];
    NSString *srt = [record objectAtIndex:[_dbManager.arrColumnNames indexOfObject:@"srt"]];
    NSString *name = [record objectAtIndex:[_dbManager.arrColumnNames indexOfObject:@"name"]];
    ...
    return [NSDictionary dictionaryWithObjectsAndKeys:taskId, KEY_TASK_ID, @(status), KEY_STATUS, @(progress), KEY_PROGRESS, url, KEY_URL, filename, KEY_FILE_NAME, image, KEY_IMAGE, srt, KEY_SRT, name, KEY_NAME, headers, KEY_HEADERS, savedDir, KEY_SAVED_DIR, [NSNumber numberWithBool:(resumable == 1)], KEY_RESUMABLE, [NSNumber numberWithBool:(showNotification == 1)], KEY_SHOW_NOTIFICATION, [NSNumber numberWithBool:(openFileFromNotification == 1)], KEY_OPEN_FILE_FROM_NOTIFICATION, @(timeCreated), KEY_TIME_CREATED, nil];
}

# pragma mark - MultiDownloader
...
- (void)enqueueMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    NSString *image = call.arguments[KEY_IMAGE];
    NSString *srt = call.arguments[KEY_SRT];
    NSString *name = call.arguments[KEY_NAME];
    ...
    NSURLSessionDownloadTask *task = [self downloadTaskWithURL:[NSURL URLWithString:urlString] fileName:fileName andImage:image andSrt:srt andName:name andSavedDir:savedDir andHeaders:headers];
    ...
    __typeof__(self) __weak weakSelf = self;
    dispatch_sync(databaseQueue, ^{
        [weakSelf addNewTask:taskId url:urlString status:STATUS_ENQUEUED progress:0 filename:fileName image:image srt:srt  name:name savedDir:shortSavedDir headers:headers resumable:NO showNotification: [showNotification boolValue] openFileFromNotification: [openFileFromNotification boolValue]];
    });
    result(taskId);
...
- (void)removeMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if (taskDict != nil) {
        ...
        if (shouldDeleteContent) {
            ...
            NSURL *imageURL = [self imageUrlFromDict:taskDict];
            NSURL *srtURL = [self srtUrlFromDict:taskDict];
            ...
            if ([fileManager fileExistsAtPath:[srtURL path]]) {
                [fileManager removeItemAtURL:srtURL error:&error];
                  if (error == nil) {
                  NSLog(@"delete content file successfully");
                  } else {
                  NSLog(@"cannot delete content file: %@", [error localizedDescription]);
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    @synchronized(self) {
        if (instance == nil) {
            instance = [[MultiDownloaderPlugin alloc] init:registrar];
