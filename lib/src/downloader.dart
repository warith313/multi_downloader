...
class MultiDownloader {
  static const _channel = const MethodChannel('com.iraqtechno/downloader');
  static bool _initialized = false;

  static Future<Null> initialize() async {
    assert(!_initialized,
        'MultiDownloader.initialize() must be called only once!');
  ...
  static Future<String> enqueue(
    ...
    assert(_initialized, 'MultiDownloader.initialize() must be called first');
  ...
  ///
  /// ```dart
  /// MultiDownloader.loadTasksWithRawQuery(query: 'SELECT * FROM task WHERE status=3');
  /// ```
  ///
